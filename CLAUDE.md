...
### WASM Backends
- **FFmpeg WASM** (`src/utils/ffmpeg.ts`): Primary audio processing. Loaded from unpkg CDN, with multi-thread support when `crossOriginIsolated`. FFmpeg logs and progress events are output to the main console (not the iframe logger) for debugging.
...
### Key Files
- `src/data.ts` — Centralized reactive state (fromData, userConfig, Lyrics/ClipRanges types)
